    ...
    <div>
      ...
      <Section icon="✂️" title="Fez Parcial?" t={t} accent="#a855f7">
        <SimNao value={f.fezParcial} onChange={v=>{
          set("fezParcial",v);
          if(!v){set("parcialRR","");set("parcialRRCustom","");set("parcialMotivoMenos","");set("parcialContratos","");set("parciais",[]);}
        ...
        {f.fezParcial===true&&(()=>{
          ...
          const addParcial=()=>{
            if(parciais.length<3) set("parciais",[...parciais,{contratos:"",pontos:""}]);
          };
          const remParcial=(idx)=>{
            const novo=parciais.filter((_,i)=>i!==idx);
            set("parciais",novo.length>0?novo:[{contratos:"",pontos:""}]);
          };

          // Calcular valor de cada parcial
          ...
          return (
            <div style={{marginTop:14}}>
              {/* ── RR global (mantido para compatibilidade) ── */}
              <div style={{background:t.bg,border:"1px solid #a855f733",borderRadius:10,padding:"14px 16px",marginBottom:10}}>
              ...
              {/* ── PARCIAIS individuais ── */}
              {isFutBR&&parciais.map((p,idx)=>{
                const cts=parseFloat(p.contratos)||0;
                const pts=parseFloat(p.pontos)||0;
                ...
                return (
                  <div key={idx} style={{background:t.bg,border:"1px solid #a855f744",borderRadius:10,padding:"14px 16px",marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <div style={{color:"#c084fc",fontWeight:800,fontSize:12}}>
                        ✂️ PARCIAL {idx+1}{parciais.length>1?` de ${parciais.length}`:""}
                      </div>
                      {parciais.length>1&&(
                        <button onClick={()=>remParcial(idx)} style={{background:"#ef444415",border:"1px solid #ef444433",borderRadius:6,color:"#f87171",fontSize:11,padding:"3px 10px",cursor:"pointer",fontWeight:700}}>✕ Remover</button>
                      )}
                    </div>
                    <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
                      ...
                      <div>
                        ...
                        <input type="number" step={isWDO?0.5:1} min="0" placeholder={`ex: ${stopPts||100}`}
                          value={p.pontos}
                          onChange={e=>updParcial(idx,"pontos",e.target.value)}
                          style={{...inp,width:110,border:"1px solid #a855f755"}}/>
                      </div>
                      {cts>0&&pts>0&&(
                        <div style={{background:"#a855f710",border:"1px solid #a855f733",borderRadius:10,padding:"10px 14px",flex:1,minWidth:140}}>
              ...
              {/* ── Botão adicionar parcial ── */}
              {isFutBR&&parciais.length<3&&(
                <button onClick={addParcial}
                  style={{width:"100%",padding:"10px 0",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,
                    border:"2px dashed #a855f766",background:"#a855f708",color:"#c084fc",transition:"all .15s",marginBottom:4}}
                >＋ Adicionar outra parcial {parciais.length>0?`(${parciais.length}/3)`:""}</button>
              )}

              {/* ── Resumo total parciais ── */}
              {isFutBR&&parciais.length>1&&totalParciais>0&&(
                <div style={{background:"#a855f715",border:"2px solid #a855f755",borderRadius:10,padding:"12px 16px",marginTop:4}}>
                  <div style={{color:"#c084fc",fontSize:11,fontWeight:700,marginBottom:6}}>📊 TOTAL DAS PARCIAIS ({parciais.length}x)</div>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap",alignItems:"center"}}>
                    {parciais.map((p,idx)=>{
                      const v=calcVlrParcial(p);
                      return v>0?(
                        <span key={idx} style={{color:"#a855f7",fontSize:12,fontWeight:600}}>P{idx+1}: +R$ {v.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}</span>
                      ):null;
                    })}
                    <span style={{color:t.muted,fontSize:13,fontWeight:700}}>=</span>
                    <span style={{color:"#4ade80",fontWeight:900,fontSize:18}}>+R$ {totalParciais.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}</span>
                  </div>
                </div>
              )}
            </div>
